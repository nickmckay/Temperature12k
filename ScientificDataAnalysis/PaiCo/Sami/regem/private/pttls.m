function [Xr, Sr, rho, eta] = pttls(V, d, missing, r, Sglobal)
...
if nargin == 2
    ...
    k       = nnz(missing);           % number of right-hand sides 
    n       = na-k;           % number of columns of A (number of variables)
end	     
...
if (numel(r) ~= 1)
    error('Exactly one truncation parameter required.');
end
...
if (nnz(~missing) < r)
    r = max(min(r,n),1);
    Sr = V(missing,r+1:end)*(repmat(d(r+1:end),1,k).*V(missing,r+1:end)');
else    
    Sr = Sglobal(missing, missing);
end
