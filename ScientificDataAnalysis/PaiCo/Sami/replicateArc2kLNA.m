...
load ~/Dropbox/Pages2k/ProxyData.mat
...
for i=1:size(nonannData,2)
    ...
    if P.Arc.TempRelate(oldi)==-1 & ~strcmp(P.Arc.RecNames{oldi},'Larsen2011')
...
load ~/Dropbox/Pages2k/arcinstrumental.mat
...
load ~/Dropbox/Pages2k/OrigRecon.mat
load ~/Dropbox/Pages2k/Sami_recons.mat

options.resample.number = 00;
...
arc.target.times = 01:2000;
arc.instrumental.noisestd = .2224;
arc.instrumental.data = fliplr((L.Arc(1:151))');
arc.instrumental.times = OrigRecon.Arc.Year(1851:end)';

lnaopts.preSamplerIterations = 500;
lnaopts.samplerIterations = 1000;
lnaopts.useModes = true;

result = lna(arc,lnaopts,@tracker);
nicksARC=(nanmean(result.signals(501:1000,:)));
%%
...
nicksARC=nicksARC-mean(nicksARC)+mean(L.Arc);

nicksARC=nicksARC./std(nicksARC).*std(L.Arc);
...
subplot(223)
plot(flipud(L.Arc(1:end-1)),nicksARC,'k.')
axis square
ylim(xlim)
hold on
plot([-5 5],[-5 5],'r')
...
subplot(223)
plot(OrigRecon.Arc.Temp(2:end),nicksARC,'k.')
axis square
ylim(xlim)
hold on
plot([-5 5],[-5 5],'r')
